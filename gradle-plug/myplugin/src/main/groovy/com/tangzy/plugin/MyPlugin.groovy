...
import com.tangzy.plugin.exten.*
import org.gradle.api.Plugin
import org.gradle.api.Project

/**
 * 解决了啥问题：一个组件，既可以application编译，也可以library编译
 * Q：原有的代码中写死了apply plugin方式，现在要交给新的脚本决定
 * Q：开发期间，组建间的关系隔离
 * => com.android.library不能写死
 * => com.android.application不能写死
 * => dependencies不能写死
 *
 * 1、针对每个project，获取编译信息
 * 2、模块如果独立编译，那么设置独立的runAlone代码、com.android.application
 *       如果子库变异，那么设置成子模块com.android.library即可
 * 3、针对当前正在编译的模块，需要去编译其依赖的模块：根据debugComponent、compileComponent来确定依赖，在dependencies中添加compile语句
 */
public class MyPlugin implements Plugin<Project> {
    ...
    void apply(Project project) {
        ...
        // Todo: 获取任务的名称assemble debug
        String taskNames = project.gradle.startParameter.taskNames.toString()
        System.out.println("DDComponent Com Build")
        System.out.println("taskNames is " + taskNames)

        // Todo: 处理一个模块Module
        String module = project.path.replace(":", "")
        System.out.println("current module is " + module + ", project.path = "+project.path)
        ...
        // 但如果是false，则不用修改
        // Todo: app和组件独立编译，isRunAlone=true
        ...
        // Todo: 独立运行设置为com.android.application；否则设置为com.android.library
        if (isRunAlone) {
